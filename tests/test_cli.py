...
def test_source_add_wizard(runner, store):
    # Simulate wizard: pick type 16 (wise), name "wise-main", token "my-token"
    # Type list is sorted, so wise is at position 16
    input_text = "16\nwise-main\nmy-token\n"
...
def test_source_add_with_defaults(runner, store):
    ...
    # Sorted types:
    # 1 binance, 2 binance_th, 3 bitget_wallet, 4 blend, 5 bybit, 6 emcd,
    # 7 ibkr, 8 kbank, 9 lobstr, 10 mexc, 11 mexc_earn, 12 okx, 13 rabby,
    # 14 revolut, 15 trading212, 16 wise, 17 yo.
